...
const ShipmentJourneyMap: React.FC<ShipmentJourneyMapProps> = ({ origin, destination, status, carrier }) => {
    const progressPercentage = Math.min(100, Math.max(0, (getStatusIndex(status) / 3) * 100));
                    ...
                    <div className="relative w-full h-full flex items-center">
                        ...
                        >
                            <div className="relative -translate-x-1/2 -translate-y-1/2">
                                {/* Radar Ping */}
                                <div className="absolute inset-0 bg-brand-primary/40 rounded-full animate-ping w-12 h-12 -m-3"></div>
                                
                                {/* Truck Icon Circle */}
                                <div className="relative w-10 h-10 bg-brand-primary rounded-full shadow-xl border-2 border-white flex items-center justify-center transform transition hover:scale-110">
                                    <Icon name={transitIcon} className="h-5 w-5 text-white" />
                                </div>
                                ...
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1 bg-slate-800 text-white text-xs font-semibold rounded-full shadow-lg whitespace-nowrap flex items-center gap-2">
                                    <span>{status === 'Delivered' ? 'Arrived' : 'On Route'}</span>
                                    {status !== 'Delivered' && <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse"></span>}
                                </div>
                            </div>
                        </div>
                    </div>
